...
class TestArm(unittest.TestCase):
  ...
  def test_arm(self):
    waistAngles = ik.solveWaist(self.target.x, self.target.y, self.shoulderWristOffset)
    expectedSets = [ [waistAngles[0], math.radians(0), 0], [waistAngles[1], math.radians(180), 0] ]
    resultSets = ik.solveArm(self.target, self.upperArmLength, self.foreArmLength, self.shoulderWristOffset, self.shoulderZ)
